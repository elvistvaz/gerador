...
@Component({
  selector: 'app-segmentoAtendido-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './segmentoAtendido-list.component.html',
  styleUrls: ['./segmentoAtendido-list.component.css']
})
export class SegmentoAtendidoListComponent implements OnInit {

  items: SegmentoAtendidoList[] = [];
  filteredItems: SegmentoAtendidoList[] = [];
  searchTerm: string = '';
  ...
  publicoAlvoMap: Map<number, string> = new Map();
  ...
  loadPublicoAlvo(): void {
    this.publicoAlvoService.findAll(0, 10000).subscribe({
      next: (data) => {
        data.content.forEach((item: PublicoAlvoList) => {
          this.publicoAlvoMap.set(item.id, String(item.nome));
        });
      },
  ...
  loadParentName(): void {
    ...
    setTimeout(() => {
      switch (this.parentFilterKey) {
        case 'publicoAlvoId':
          this.parentName = this.publicoAlvoMap.get(parentId) || null;
          break;
      }
    }, 500);
  }
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/segmentoAtendido/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/segmentoAtendido/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/segmentoAtendido/editar', id]);
  }
