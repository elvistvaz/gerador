...
import { ConceitoAprendidoRequest, ConceitoAprendidoResponse } from '../models/conceitoAprendido.model';
import { ConceitoAprendidoService } from '../services/conceitoAprendido.service';

@Component({
  selector: 'app-conceitoAprendido-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './conceitoAprendido-form.component.html',
  styleUrls: ['./conceitoAprendido-form.component.css']
})
export class ConceitoAprendidoFormComponent implements OnInit {

  item: ConceitoAprendidoRequest = {} as ConceitoAprendidoRequest;
  ...
  constructor(
    private service: ConceitoAprendidoService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as ConceitoAprendidoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/conceitoAprendido'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/conceitoAprendido']);
    }
  }
