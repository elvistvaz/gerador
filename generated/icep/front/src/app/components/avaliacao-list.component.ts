...
export class AvaliacaoListComponent implements OnInit {
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/avaliacao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/avaliacao/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/avaliacao/editar', id]);
  }
