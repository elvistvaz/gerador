...
import { PublicoAlvoRequest, PublicoAlvoResponse } from '../models/publicoAlvo.model';
import { PublicoAlvoService } from '../services/publicoAlvo.service';

@Component({
  selector: 'app-publicoAlvo-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './publicoAlvo-form.component.html',
  styleUrls: ['./publicoAlvo-form.component.css']
})
export class PublicoAlvoFormComponent implements OnInit {

  item: PublicoAlvoRequest = {} as PublicoAlvoRequest;
  ...
  constructor(
    private service: PublicoAlvoService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as PublicoAlvoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/publicoAlvo'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/publicoAlvo']);
    }
  }
