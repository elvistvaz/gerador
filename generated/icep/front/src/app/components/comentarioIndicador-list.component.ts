...
import { ComentarioIndicadorList } from '../models/comentarioIndicador.model';
...
@Component({
  selector: 'app-comentarioIndicador-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './comentarioIndicador-list.component.html',
  styleUrls: ['./comentarioIndicador-list.component.css']
})
export class ComentarioIndicadorListComponent implements OnInit {

  items: ComentarioIndicadorList[] = [];
  filteredItems: ComentarioIndicadorList[] = [];
  searchTerm: string = '';
  ...
  ambitoGestaoMap: Map<number, string> = new Map();
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.loadAmbitoGestao();
      this.loadParentName();
      this.loadItems();
    });
  }

  loadAmbitoGestao(): void {
    this.ambitoGestaoService.findAll(0, 10000).subscribe({
      next: (data) => {
        data.content.forEach((item: AmbitoGestaoList) => {
          this.ambitoGestaoMap.set(item.id, String(item.nome));
        });
      },
  ...
  loadParentName(): void {
    ...
    setTimeout(() => {
      switch (this.parentFilterKey) {
        case 'ambitoGestaoId':
          this.parentName = this.ambitoGestaoMap.get(parentId) || null;
          break;
      }
    }, 500);
  }
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/comentarioIndicador/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/comentarioIndicador/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/comentarioIndicador/editar', id]);
  }
