...
import { FormacaoRequest, FormacaoResponse } from '../models/formacao.model';
...
@Component({
  ...
  styleUrls: ['./formacao-form.component.css']
})
export class FormacaoFormComponent implements OnInit {

  item: FormacaoRequest = {} as FormacaoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as FormacaoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/formacao'], { queryParams: this.returnQueryParams });
    } else {
