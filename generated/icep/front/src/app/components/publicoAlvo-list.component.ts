...
@Component({
  selector: 'app-publicoAlvo-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './publicoAlvo-list.component.html',
  styleUrls: ['./publicoAlvo-list.component.css']
})
export class PublicoAlvoListComponent implements OnInit {

  items: PublicoAlvoList[] = [];
  filteredItems: PublicoAlvoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/publicoAlvo/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/publicoAlvo/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/publicoAlvo/editar', id]);
  }
