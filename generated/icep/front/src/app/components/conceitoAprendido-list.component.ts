...
@Component({
  selector: 'app-conceitoAprendido-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './conceitoAprendido-list.component.html',
  styleUrls: ['./conceitoAprendido-list.component.css']
})
export class ConceitoAprendidoListComponent implements OnInit {

  items: ConceitoAprendidoList[] = [];
  filteredItems: ConceitoAprendidoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/conceitoAprendido/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/conceitoAprendido/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/conceitoAprendido/editar', id]);
  }
