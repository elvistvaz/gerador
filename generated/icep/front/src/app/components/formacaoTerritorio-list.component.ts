...
@Component({
  selector: 'app-formacaoTerritorio-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './formacaoTerritorio-list.component.html',
  styleUrls: ['./formacaoTerritorio-list.component.css']
})
export class FormacaoTerritorioListComponent implements OnInit {

  items: FormacaoTerritorioList[] = [];
  filteredItems: FormacaoTerritorioList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/formacaoTerritorio/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/formacaoTerritorio/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/formacaoTerritorio/editar', id]);
  }
