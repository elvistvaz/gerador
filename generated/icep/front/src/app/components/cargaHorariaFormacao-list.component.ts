...
import { CargaHorariaFormacaoList } from '../models/cargaHorariaFormacao.model';
...
@Component({
  selector: 'app-cargaHorariaFormacao-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './cargaHorariaFormacao-list.component.html',
  styleUrls: ['./cargaHorariaFormacao-list.component.css']
})
export class CargaHorariaFormacaoListComponent implements OnInit {

  items: CargaHorariaFormacaoList[] = [];
  filteredItems: CargaHorariaFormacaoList[] = [];
  searchTerm: string = '';
  ...
  formacaoMap: Map<number, string> = new Map();
  ...
  loadFormacao(): void {
    this.formacaoService.findAll(0, 10000).subscribe({
      next: (data) => {
        data.content.forEach((item: FormacaoList) => {
          this.formacaoMap.set(item.id, String(item.nome));
        });
      },
  ...
  loadParentName(): void {
    ...
    setTimeout(() => {
      switch (this.parentFilterKey) {
        case 'formacaoId':
          this.parentName = this.formacaoMap.get(parentId) || null;
          break;
      }
    }, 500);
  }
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/cargaHorariaFormacao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/cargaHorariaFormacao/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/cargaHorariaFormacao/editar', id]);
  }
