...
@Component({
  selector: 'app-ambitoGestao-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ambitoGestao-list.component.html',
  styleUrls: ['./ambitoGestao-list.component.css']
})
export class AmbitoGestaoListComponent implements OnInit {

  items: AmbitoGestaoList[] = [];
  filteredItems: AmbitoGestaoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/ambitoGestao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/ambitoGestao/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/ambitoGestao/editar', id]);
  }
