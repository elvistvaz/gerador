...
import { FormacaoTerritorioRequest, FormacaoTerritorioResponse } from '../models/formacaoTerritorio.model';
import { FormacaoTerritorioService } from '../services/formacaoTerritorio.service';

@Component({
  selector: 'app-formacaoTerritorio-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './formacaoTerritorio-form.component.html',
  styleUrls: ['./formacaoTerritorio-form.component.css']
})
export class FormacaoTerritorioFormComponent implements OnInit {

  item: FormacaoTerritorioRequest = {} as FormacaoTerritorioRequest;
  ...
  constructor(
    private service: FormacaoTerritorioService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as FormacaoTerritorioRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/formacaoTerritorio'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/formacaoTerritorio']);
    }
  }
