...
import { TerritorioRequest, TerritorioResponse } from '../models/territorio.model';
...
@Component({
  ...
  styleUrls: ['./territorio-form.component.css']
})
export class TerritorioFormComponent implements OnInit {

  item: TerritorioRequest = {} as TerritorioRequest;
  ...
  constructor(
    private service: TerritorioService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as TerritorioRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/territorio'], { queryParams: this.returnQueryParams });
    } else {
