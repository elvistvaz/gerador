...
import { AtendimentoMunicipioList } from '../models/atendimentoMunicipio.model';
...
@Component({
  selector: 'app-atendimentoMunicipio-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './atendimentoMunicipio-list.component.html',
  styleUrls: ['./atendimentoMunicipio-list.component.css']
})
export class AtendimentoMunicipioListComponent implements OnInit {

  items: AtendimentoMunicipioList[] = [];
  filteredItems: AtendimentoMunicipioList[] = [];
  searchTerm: string = '';
  ...
  segmentoAtendidoMap: Map<number, string> = new Map();
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      this.loadSegmentoAtendido();
      this.loadParentName();
      this.loadItems();
    });
  }

  loadSegmentoAtendido(): void {
    this.segmentoAtendidoService.findAll(0, 10000).subscribe({
      next: (data) => {
        data.content.forEach((item: SegmentoAtendidoList) => {
          this.segmentoAtendidoMap.set(item.id, String(item.nome));
        });
      },
  ...
  loadParentName(): void {
    ...
    setTimeout(() => {
      switch (this.parentFilterKey) {
        case 'segmentoAtendidoId':
          this.parentName = this.segmentoAtendidoMap.get(parentId) || null;
          break;
      }
    }, 500);
  }
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/atendimentoMunicipio/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/atendimentoMunicipio/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/atendimentoMunicipio/editar', id]);
  }
