...
export class TerritorioListComponent implements OnInit {
  ...
  items: TerritorioList[] = [];
  filteredItems: TerritorioList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/territorio/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/territorio/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/territorio/editar', id]);
  }
