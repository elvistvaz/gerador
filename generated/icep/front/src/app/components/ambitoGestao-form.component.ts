...
import { AmbitoGestaoRequest, AmbitoGestaoResponse } from '../models/ambitoGestao.model';
import { AmbitoGestaoService } from '../services/ambitoGestao.service';

@Component({
  selector: 'app-ambitoGestao-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './ambitoGestao-form.component.html',
  styleUrls: ['./ambitoGestao-form.component.css']
})
export class AmbitoGestaoFormComponent implements OnInit {

  item: AmbitoGestaoRequest = {} as AmbitoGestaoRequest;
  ...
  constructor(
    private service: AmbitoGestaoService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as AmbitoGestaoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/ambitoGestao'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/ambitoGestao']);
    }
  }
