...
import { ComponenteCurricularRequest, ComponenteCurricularResponse } from '../models/componenteCurricular.model';
import { ComponenteCurricularService } from '../services/componenteCurricular.service';

@Component({
  selector: 'app-componenteCurricular-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './componenteCurricular-form.component.html',
  styleUrls: ['./componenteCurricular-form.component.css']
})
export class ComponenteCurricularFormComponent implements OnInit {

  item: ComponenteCurricularRequest = {} as ComponenteCurricularRequest;
  ...
  constructor(
    private service: ComponenteCurricularService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as ComponenteCurricularRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/componenteCurricular'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/componenteCurricular']);
    }
  }
