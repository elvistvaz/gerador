...
@Component({
  selector: 'app-componenteCurricular-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './componenteCurricular-list.component.html',
  styleUrls: ['./componenteCurricular-list.component.css']
})
export class ComponenteCurricularListComponent implements OnInit {

  items: ComponenteCurricularList[] = [];
  filteredItems: ComponenteCurricularList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/componenteCurricular/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/componenteCurricular/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/componenteCurricular/editar', id]);
  }
