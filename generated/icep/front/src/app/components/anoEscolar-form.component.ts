...
import { AnoEscolarRequest, AnoEscolarResponse } from '../models/anoEscolar.model';
import { AnoEscolarService } from '../services/anoEscolar.service';

@Component({
  selector: 'app-anoEscolar-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './anoEscolar-form.component.html',
  styleUrls: ['./anoEscolar-form.component.css']
})
export class AnoEscolarFormComponent implements OnInit {

  item: AnoEscolarRequest = {} as AnoEscolarRequest;
  ...
  constructor(
    private service: AnoEscolarService,
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as AnoEscolarRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/anoEscolar'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/anoEscolar']);
    }
  }
