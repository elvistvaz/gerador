...
@Component({
  selector: 'app-anoEscolar-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './anoEscolar-list.component.html',
  styleUrls: ['./anoEscolar-list.component.css']
})
export class AnoEscolarListComponent implements OnInit {

  items: AnoEscolarList[] = [];
  filteredItems: AnoEscolarList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/anoEscolar/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/anoEscolar/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/anoEscolar/editar', id]);
  }
