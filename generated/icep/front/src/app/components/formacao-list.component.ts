...
@Component({
  ...
  styleUrls: ['./formacao-list.component.css']
})
export class FormacaoListComponent implements OnInit {

  items: FormacaoList[] = [];
  filteredItems: FormacaoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/formacao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/formacao/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/formacao/editar', id]);
  }
