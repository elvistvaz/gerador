...
import { IndicacaoRequest, IndicacaoResponse } from '../models/indicacao.model';
...
@Component({
  ...
  styleUrls: ['./indicacao-form.component.css']
})
export class IndicacaoFormComponent implements OnInit {

  item: IndicacaoRequest = {} as IndicacaoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as IndicacaoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/indicacao'], { queryParams: this.returnQueryParams });
    } else {
