...
import { OperadoraRequest, OperadoraResponse } from '../models/operadora.model';
...
@Component({
  ...
  styleUrls: ['./operadora-form.component.css']
})
export class OperadoraFormComponent implements OnInit {

  item: OperadoraRequest = {} as OperadoraRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as OperadoraRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/operadora'], { queryParams: this.returnQueryParams });
    } else {
