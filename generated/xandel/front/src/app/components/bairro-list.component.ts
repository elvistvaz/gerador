...
import { BairroList } from '../models/bairro.model';
...
export class BairroListComponent implements OnInit {
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/bairro/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/bairro/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/bairro/editar', id]);
  }
