...
import { TipoServicoList } from '../models/tipoServico.model';
...
export class TipoServicoListComponent implements OnInit {
  ...
  items: TipoServicoList[] = [];
  filteredItems: TipoServicoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/tipoServico/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/tipoServico/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/tipoServico/editar', id]);
  }
