...
import { ParametroNFRequest, ParametroNFResponse } from '../models/parametroNF.model';
import { ParametroNFService } from '../services/parametroNF.service';

@Component({
  selector: 'app-parametroNF-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './parametroNF-form.component.html',
  styleUrls: ['./parametroNF-form.component.css']
})
export class ParametroNFFormComponent implements OnInit {

  item: ParametroNFRequest = {} as ParametroNFRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as ParametroNFRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/parametroNF'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/parametroNF']);
    }
  }
