...
import { LancamentoList } from '../models/lancamento.model';
...
export class LancamentoListComponent implements OnInit {
  ...
  items: LancamentoList[] = [];
  filteredItems: LancamentoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/lancamento/novo'], { queryParams: this.parentFilter });
    } else {
