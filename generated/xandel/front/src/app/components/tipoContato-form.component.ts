...
import { TipoContatoRequest, TipoContatoResponse } from '../models/tipoContato.model';
import { TipoContatoService } from '../services/tipoContato.service';

@Component({
  selector: 'app-tipoContato-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './tipoContato-form.component.html',
  styleUrls: ['./tipoContato-form.component.css']
})
export class TipoContatoFormComponent implements OnInit {

  item: TipoContatoRequest = {} as TipoContatoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as TipoContatoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/tipoContato'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/tipoContato']);
    }
  }
