...
import { EmpresaDespesaFixaList, EmpresaDespesaFixaId } from '../models/empresaDespesaFixa.model';
import { EmpresaDespesaFixaService } from '../services/empresaDespesaFixa.service';

@Component({
  selector: 'app-empresaDespesaFixa-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './empresaDespesaFixa-list.component.html',
  styleUrls: ['./empresaDespesaFixa-list.component.css']
})
export class EmpresaDespesaFixaListComponent implements OnInit {

  items: EmpresaDespesaFixaList[] = [];
  filteredItems: EmpresaDespesaFixaList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: EmpresaDespesaFixaService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/empresaDespesaFixa/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/empresaDespesaFixa/novo']);
    }
  }

  editar(id: EmpresaDespesaFixaId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/empresaDespesaFixa/editar', idParam]);
  }

  delete(id: EmpresaDespesaFixaId): void {
