...
import { PessoaCartorioList, PessoaCartorioId } from '../models/pessoaCartorio.model';
import { PessoaCartorioService } from '../services/pessoaCartorio.service';

@Component({
  selector: 'app-pessoaCartorio-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pessoaCartorio-list.component.html',
  styleUrls: ['./pessoaCartorio-list.component.css']
})
export class PessoaCartorioListComponent implements OnInit {

  items: PessoaCartorioList[] = [];
  filteredItems: PessoaCartorioList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: PessoaCartorioService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/pessoaCartorio/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/pessoaCartorio/novo']);
    }
  }

  editar(id: PessoaCartorioId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/pessoaCartorio/editar', idParam]);
  }

  delete(id: PessoaCartorioId): void {
