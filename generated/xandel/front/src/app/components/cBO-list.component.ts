...
import { CBOList } from '../models/cBO.model';
import { CBOService } from '../services/cBO.service';

@Component({
  selector: 'app-cBO-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './cBO-list.component.html',
  styleUrls: ['./cBO-list.component.css']
})
export class CBOListComponent implements OnInit {

  items: CBOList[] = [];
  filteredItems: CBOList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/cBO/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/cBO/novo']);
    }
  }
