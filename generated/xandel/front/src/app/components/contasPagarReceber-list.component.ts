...
import { ContasPagarReceberList } from '../models/contasPagarReceber.model';
import { ContasPagarReceberService } from '../services/contasPagarReceber.service';
import { SessionService } from '../services/session.service';

@Component({
  selector: 'app-contasPagarReceber-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './contasPagarReceber-list.component.html',
  styleUrls: ['./contasPagarReceber-list.component.css']
})
export class ContasPagarReceberListComponent implements OnInit {

  items: ContasPagarReceberList[] = [];
  filteredItems: ContasPagarReceberList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/contasPagarReceber/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/contasPagarReceber/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/contasPagarReceber/editar', id]);
  }
