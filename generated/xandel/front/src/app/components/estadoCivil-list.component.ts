...
import { EstadoCivilList } from '../models/estadoCivil.model';
import { EstadoCivilService } from '../services/estadoCivil.service';

@Component({
  selector: 'app-estadoCivil-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './estadoCivil-list.component.html',
  styleUrls: ['./estadoCivil-list.component.css']
})
export class EstadoCivilListComponent implements OnInit {

  items: EstadoCivilList[] = [];
  filteredItems: EstadoCivilList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/estadoCivil/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/estadoCivil/novo']);
    }
  }
