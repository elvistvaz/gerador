...
import { EmpresaClienteRequest, EmpresaClienteResponse } from '../models/empresaCliente.model';
import { EmpresaClienteService } from '../services/empresaCliente.service';

@Component({
  selector: 'app-empresaCliente-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './empresaCliente-form.component.html',
  styleUrls: ['./empresaCliente-form.component.css']
})
export class EmpresaClienteFormComponent implements OnInit {

  item: EmpresaClienteRequest = {} as EmpresaClienteRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as EmpresaClienteRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/empresaCliente'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/empresaCliente']);
    }
  }
