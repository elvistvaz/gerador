...
import { IndicacaoList } from '../models/indicacao.model';
...
export class IndicacaoListComponent implements OnInit {
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/indicacao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/indicacao/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/indicacao/editar', id]);
  }
