...
import { BancoRequest, BancoResponse } from '../models/banco.model';
...
export class BancoFormComponent implements OnInit {
  ...
  item: BancoRequest = {} as BancoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as BancoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/banco'], { queryParams: this.returnQueryParams });
    } else {
