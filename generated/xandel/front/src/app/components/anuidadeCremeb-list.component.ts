...
import { AnuidadeCremebList } from '../models/anuidadeCremeb.model';
import { AnuidadeCremebService } from '../services/anuidadeCremeb.service';
import { SessionService } from '../services/session.service';

@Component({
  selector: 'app-anuidadeCremeb-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './anuidadeCremeb-list.component.html',
  styleUrls: ['./anuidadeCremeb-list.component.css']
})
export class AnuidadeCremebListComponent implements OnInit {

  items: AnuidadeCremebList[] = [];
  filteredItems: AnuidadeCremebList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/anuidadeCremeb/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/anuidadeCremeb/novo']);
    }
  }
