...
import { TipoServicoRequest, TipoServicoResponse } from '../models/tipoServico.model';
import { TipoServicoService } from '../services/tipoServico.service';

@Component({
  selector: 'app-tipoServico-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './tipoServico-form.component.html',
  styleUrls: ['./tipoServico-form.component.css']
})
export class TipoServicoFormComponent implements OnInit {

  item: TipoServicoRequest = {} as TipoServicoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as TipoServicoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/tipoServico'], { queryParams: this.returnQueryParams });
    } else {
