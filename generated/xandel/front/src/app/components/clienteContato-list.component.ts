...
import { ClienteContatoList } from '../models/clienteContato.model';
import { ClienteContatoService } from '../services/clienteContato.service';

@Component({
  selector: 'app-clienteContato-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './clienteContato-list.component.html',
  styleUrls: ['./clienteContato-list.component.css']
})
export class ClienteContatoListComponent implements OnInit {

  items: ClienteContatoList[] = [];
  filteredItems: ClienteContatoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/clienteContato/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/clienteContato/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/clienteContato/editar', id]);
  }
