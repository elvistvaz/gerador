...
import { CartorioList } from '../models/cartorio.model';
import { CartorioService } from '../services/cartorio.service';

@Component({
  selector: 'app-cartorio-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './cartorio-list.component.html',
  styleUrls: ['./cartorio-list.component.css']
})
export class CartorioListComponent implements OnInit {

  items: CartorioList[] = [];
  filteredItems: CartorioList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: CartorioService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/cartorio/novo'], { queryParams: this.parentFilter });
    } else {
  ...
  editar(id: number): void {
    this.router.navigate(['/cartorio/editar', id]);
  }
