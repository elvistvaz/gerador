...
import { AnuidadeCremebItemList } from '../models/anuidadeCremebItem.model';
import { AnuidadeCremebItemService } from '../services/anuidadeCremebItem.service';

@Component({
  selector: 'app-anuidadeCremebItem-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './anuidadeCremebItem-list.component.html',
  styleUrls: ['./anuidadeCremebItem-list.component.css']
})
export class AnuidadeCremebItemListComponent implements OnInit {

  items: AnuidadeCremebItemList[] = [];
  filteredItems: AnuidadeCremebItemList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: AnuidadeCremebItemService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/anuidadeCremebItem/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/anuidadeCremebItem/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/anuidadeCremebItem/editar', id]);
  }
