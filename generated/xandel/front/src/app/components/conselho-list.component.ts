...
import { ConselhoList } from '../models/conselho.model';
import { ConselhoService } from '../services/conselho.service';

@Component({
  selector: 'app-conselho-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './conselho-list.component.html',
  styleUrls: ['./conselho-list.component.css']
})
export class ConselhoListComponent implements OnInit {

  items: ConselhoList[] = [];
  filteredItems: ConselhoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/conselho/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/conselho/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/conselho/editar', id]);
  }
