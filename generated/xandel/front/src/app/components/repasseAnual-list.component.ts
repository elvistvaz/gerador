...
import { RepasseAnualList, RepasseAnualId } from '../models/repasseAnual.model';
import { RepasseAnualService } from '../services/repasseAnual.service';
import { SessionService } from '../services/session.service';

@Component({
  selector: 'app-repasseAnual-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './repasseAnual-list.component.html',
  styleUrls: ['./repasseAnual-list.component.css']
})
export class RepasseAnualListComponent implements OnInit {

  items: RepasseAnualList[] = [];
  filteredItems: RepasseAnualList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/repasseAnual/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/repasseAnual/novo']);
    }
  }

  editar(id: RepasseAnualId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/repasseAnual/editar', idParam]);
  }

  delete(id: RepasseAnualId): void {
