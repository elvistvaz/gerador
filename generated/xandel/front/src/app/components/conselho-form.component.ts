...
import { ConselhoRequest, ConselhoResponse } from '../models/conselho.model';
...
@Component({
  ...
  templateUrl: './conselho-form.component.html',
  styleUrls: ['./conselho-form.component.css']
})
export class ConselhoFormComponent implements OnInit {

  item: ConselhoRequest = {} as ConselhoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as ConselhoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/conselho'], { queryParams: this.returnQueryParams });
    } else {
