...
import { DespesaReceitaList } from '../models/despesaReceita.model';
import { DespesaReceitaService } from '../services/despesaReceita.service';

@Component({
  selector: 'app-despesaReceita-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './despesaReceita-list.component.html',
  styleUrls: ['./despesaReceita-list.component.css']
})
export class DespesaReceitaListComponent implements OnInit {

  items: DespesaReceitaList[] = [];
  filteredItems: DespesaReceitaList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: DespesaReceitaService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/despesaReceita/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/despesaReceita/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/despesaReceita/editar', id]);
  }
