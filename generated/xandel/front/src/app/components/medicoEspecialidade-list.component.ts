...
import { MedicoEspecialidadeList, MedicoEspecialidadeId } from '../models/medicoEspecialidade.model';
import { MedicoEspecialidadeService } from '../services/medicoEspecialidade.service';

@Component({
  selector: 'app-medicoEspecialidade-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './medicoEspecialidade-list.component.html',
  styleUrls: ['./medicoEspecialidade-list.component.css']
})
export class MedicoEspecialidadeListComponent implements OnInit {

  items: MedicoEspecialidadeList[] = [];
  filteredItems: MedicoEspecialidadeList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/medicoEspecialidade/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/medicoEspecialidade/novo']);
    }
  }

  editar(id: MedicoEspecialidadeId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/medicoEspecialidade/editar', idParam]);
  }

  delete(id: MedicoEspecialidadeId): void {
