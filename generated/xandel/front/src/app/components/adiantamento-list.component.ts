...
import { AdiantamentoList } from '../models/adiantamento.model';
import { AdiantamentoService } from '../services/adiantamento.service';
import { SessionService } from '../services/session.service';

@Component({
  selector: 'app-adiantamento-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './adiantamento-list.component.html',
  styleUrls: ['./adiantamento-list.component.css']
})
export class AdiantamentoListComponent implements OnInit {

  items: AdiantamentoList[] = [];
  filteredItems: AdiantamentoList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: AdiantamentoService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/adiantamento/novo'], { queryParams: this.parentFilter });
    } else {
  ...
  editar(id: number): void {
    this.router.navigate(['/adiantamento/editar', id]);
  }
