...
import { BairroRequest, BairroResponse } from '../models/bairro.model';
...
export class BairroFormComponent implements OnInit {
  ...
  item: BairroRequest = {} as BairroRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as BairroRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/bairro'], { queryParams: this.returnQueryParams });
    } else {
