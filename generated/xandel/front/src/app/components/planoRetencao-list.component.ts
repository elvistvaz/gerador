...
import { PlanoRetencaoList } from '../models/planoRetencao.model';
import { PlanoRetencaoService } from '../services/planoRetencao.service';

@Component({
  selector: 'app-planoRetencao-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './planoRetencao-list.component.html',
  styleUrls: ['./planoRetencao-list.component.css']
})
export class PlanoRetencaoListComponent implements OnInit {

  items: PlanoRetencaoList[] = [];
  filteredItems: PlanoRetencaoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/planoRetencao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/planoRetencao/novo']);
    }
  }
