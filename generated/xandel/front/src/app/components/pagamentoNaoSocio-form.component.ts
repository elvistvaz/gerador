...
import { PagamentoNaoSocioRequest, PagamentoNaoSocioResponse } from '../models/pagamentoNaoSocio.model';
import { PagamentoNaoSocioService } from '../services/pagamentoNaoSocio.service';
import { SessionService } from '../services/session.service';

@Component({
  selector: 'app-pagamentoNaoSocio-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pagamentoNaoSocio-form.component.html',
  styleUrls: ['./pagamentoNaoSocio-form.component.css']
})
export class PagamentoNaoSocioFormComponent implements OnInit {

  item: PagamentoNaoSocioRequest = {} as PagamentoNaoSocioRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as PagamentoNaoSocioRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/pagamentoNaoSocio'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/pagamentoNaoSocio']);
    }
  }
