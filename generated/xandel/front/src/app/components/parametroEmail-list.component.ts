...
import { ParametroEmailList } from '../models/parametroEmail.model';
import { ParametroEmailService } from '../services/parametroEmail.service';

@Component({
  selector: 'app-parametroEmail-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './parametroEmail-list.component.html',
  styleUrls: ['./parametroEmail-list.component.css']
})
export class ParametroEmailListComponent implements OnInit {

  items: ParametroEmailList[] = [];
  filteredItems: ParametroEmailList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: ParametroEmailService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/parametroEmail/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/parametroEmail/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/parametroEmail/editar', id]);
  }
