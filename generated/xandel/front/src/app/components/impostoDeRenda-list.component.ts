...
import { ImpostoDeRendaList, ImpostoDeRendaId } from '../models/impostoDeRenda.model';
import { ImpostoDeRendaService } from '../services/impostoDeRenda.service';

@Component({
  selector: 'app-impostoDeRenda-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './impostoDeRenda-list.component.html',
  styleUrls: ['./impostoDeRenda-list.component.css']
})
export class ImpostoDeRendaListComponent implements OnInit {

  items: ImpostoDeRendaList[] = [];
  filteredItems: ImpostoDeRendaList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: ImpostoDeRendaService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/impostoDeRenda/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/impostoDeRenda/novo']);
    }
  }

  editar(id: ImpostoDeRendaId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/impostoDeRenda/editar', idParam]);
  }

  delete(id: ImpostoDeRendaId): void {
