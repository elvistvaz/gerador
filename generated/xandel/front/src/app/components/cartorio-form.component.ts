...
import { CartorioRequest, CartorioResponse } from '../models/cartorio.model';
...
export class CartorioFormComponent implements OnInit {

  item: CartorioRequest = {} as CartorioRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as CartorioRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/cartorio'], { queryParams: this.returnQueryParams });
    } else {
