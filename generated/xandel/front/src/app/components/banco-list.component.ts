...
import { BancoList } from '../models/banco.model';
...
export class BancoListComponent implements OnInit {
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/banco/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/banco/novo']);
    }
  }
