...
import { EmpresaSocioList, EmpresaSocioId } from '../models/empresaSocio.model';
import { EmpresaSocioService } from '../services/empresaSocio.service';

@Component({
  selector: 'app-empresaSocio-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './empresaSocio-list.component.html',
  styleUrls: ['./empresaSocio-list.component.css']
})
export class EmpresaSocioListComponent implements OnInit {

  items: EmpresaSocioList[] = [];
  filteredItems: EmpresaSocioList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: EmpresaSocioService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/empresaSocio/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/empresaSocio/novo']);
    }
  }

  editar(id: EmpresaSocioId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/empresaSocio/editar', idParam]);
  }

  delete(id: EmpresaSocioId): void {
