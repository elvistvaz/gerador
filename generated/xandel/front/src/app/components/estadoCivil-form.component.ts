...
import { EstadoCivilRequest, EstadoCivilResponse } from '../models/estadoCivil.model';
import { EstadoCivilService } from '../services/estadoCivil.service';

@Component({
  selector: 'app-estadoCivil-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './estadoCivil-form.component.html',
  styleUrls: ['./estadoCivil-form.component.css']
})
export class EstadoCivilFormComponent implements OnInit {

  item: EstadoCivilRequest = {} as EstadoCivilRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as EstadoCivilRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/estadoCivil'], { queryParams: this.returnQueryParams });
    } else {
