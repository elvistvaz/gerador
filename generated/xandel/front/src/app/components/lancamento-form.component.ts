...
import { LancamentoRequest, LancamentoResponse } from '../models/lancamento.model';
...
export class LancamentoFormComponent implements OnInit {
  ...
  item: LancamentoRequest = {} as LancamentoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as LancamentoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/lancamento'], { queryParams: this.returnQueryParams });
    } else {
