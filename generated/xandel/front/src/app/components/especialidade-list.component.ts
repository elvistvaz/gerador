...
import { EspecialidadeList } from '../models/especialidade.model';
...
export class EspecialidadeListComponent implements OnInit {
  ...
  filteredItems: EspecialidadeList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/especialidade/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/especialidade/novo']);
    }
  }
