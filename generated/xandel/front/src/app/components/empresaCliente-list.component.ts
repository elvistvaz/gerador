...
import { EmpresaClienteList, EmpresaClienteId } from '../models/empresaCliente.model';
import { EmpresaClienteService } from '../services/empresaCliente.service';

@Component({
  selector: 'app-empresaCliente-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './empresaCliente-list.component.html',
  styleUrls: ['./empresaCliente-list.component.css']
})
export class EmpresaClienteListComponent implements OnInit {

  items: EmpresaClienteList[] = [];
  filteredItems: EmpresaClienteList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/empresaCliente/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/empresaCliente/novo']);
    }
  }

  editar(id: EmpresaClienteId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/empresaCliente/editar', idParam]);
  }

  delete(id: EmpresaClienteId): void {
