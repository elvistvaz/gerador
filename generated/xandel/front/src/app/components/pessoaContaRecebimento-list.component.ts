...
import { PessoaContaRecebimentoList, PessoaContaRecebimentoId } from '../models/pessoaContaRecebimento.model';
import { PessoaContaRecebimentoService } from '../services/pessoaContaRecebimento.service';

@Component({
  selector: 'app-pessoaContaRecebimento-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pessoaContaRecebimento-list.component.html',
  styleUrls: ['./pessoaContaRecebimento-list.component.css']
})
export class PessoaContaRecebimentoListComponent implements OnInit {

  items: PessoaContaRecebimentoList[] = [];
  filteredItems: PessoaContaRecebimentoList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: PessoaContaRecebimentoService,
  ...
  loadItems(): void {
    this.loading = true;
    this.error = null;
    this.service.findAll(this.currentPage, this.pageSize).subscribe({
      next: (data) => {
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/pessoaContaRecebimento/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/pessoaContaRecebimento/novo']);
    }
  }

  editar(id: PessoaContaRecebimentoId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/pessoaContaRecebimento/editar', idParam]);
  }

  delete(id: PessoaContaRecebimentoId): void {
