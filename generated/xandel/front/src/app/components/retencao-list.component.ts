...
import { RetencaoList, RetencaoId } from '../models/retencao.model';
import { RetencaoService } from '../services/retencao.service';

@Component({
  selector: 'app-retencao-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './retencao-list.component.html',
  styleUrls: ['./retencao-list.component.css']
})
export class RetencaoListComponent implements OnInit {

  items: RetencaoList[] = [];
  filteredItems: RetencaoList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: RetencaoService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/retencao/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/retencao/novo']);
    }
  }

  editar(id: RetencaoId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/retencao/editar', idParam]);
  }

  delete(id: RetencaoId): void {
