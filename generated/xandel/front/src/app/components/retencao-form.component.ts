...
import { RetencaoRequest, RetencaoResponse } from '../models/retencao.model';
import { RetencaoService } from '../services/retencao.service';

@Component({
  selector: 'app-retencao-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './retencao-form.component.html',
  styleUrls: ['./retencao-form.component.css']
})
export class RetencaoFormComponent implements OnInit {

  item: RetencaoRequest = {} as RetencaoRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as RetencaoRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/retencao'], { queryParams: this.returnQueryParams });
    } else {
      this.router.navigate(['/retencao']);
    }
  }
