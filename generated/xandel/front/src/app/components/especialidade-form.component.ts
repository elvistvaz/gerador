...
import { EspecialidadeRequest, EspecialidadeResponse } from '../models/especialidade.model';
...
export class EspecialidadeFormComponent implements OnInit {
  ...
  item: EspecialidadeRequest = {} as EspecialidadeRequest;
  ...
  loadItem(): void {
    ...
    this.service.findById(this.id).subscribe({
      next: (data) => {
        this.item = data as EspecialidadeRequest;
        this.loading = false;
      },
      error: (err) => {
  ...
  private navegarParaLista(): void {
    if (Object.keys(this.returnQueryParams).length > 0) {
      this.router.navigate(['/especialidade'], { queryParams: this.returnQueryParams });
    } else {
