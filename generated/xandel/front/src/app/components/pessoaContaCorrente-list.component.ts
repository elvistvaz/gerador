...
import { PessoaContaCorrenteList } from '../models/pessoaContaCorrente.model';
import { PessoaContaCorrenteService } from '../services/pessoaContaCorrente.service';

@Component({
  selector: 'app-pessoaContaCorrente-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './pessoaContaCorrente-list.component.html',
  styleUrls: ['./pessoaContaCorrente-list.component.css']
})
export class PessoaContaCorrenteListComponent implements OnInit {

  items: PessoaContaCorrenteList[] = [];
  filteredItems: PessoaContaCorrenteList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: PessoaContaCorrenteService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/pessoaContaCorrente/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/pessoaContaCorrente/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/pessoaContaCorrente/editar', id]);
  }
