...
import { ClienteFilialList } from '../models/clienteFilial.model';
import { ClienteFilialService } from '../services/clienteFilial.service';

@Component({
  selector: 'app-clienteFilial-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './clienteFilial-list.component.html',
  styleUrls: ['./clienteFilial-list.component.css']
})
export class ClienteFilialListComponent implements OnInit {

  items: ClienteFilialList[] = [];
  filteredItems: ClienteFilialList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: ClienteFilialService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/clienteFilial/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/clienteFilial/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/clienteFilial/editar', id]);
  }
