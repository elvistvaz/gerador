...
import { TipoContatoList } from '../models/tipoContato.model';
import { TipoContatoService } from '../services/tipoContato.service';

@Component({
  selector: 'app-tipoContato-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './tipoContato-list.component.html',
  styleUrls: ['./tipoContato-list.component.css']
})
export class TipoContatoListComponent implements OnInit {

  items: TipoContatoList[] = [];
  filteredItems: TipoContatoList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/tipoContato/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/tipoContato/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/tipoContato/editar', id]);
  }
