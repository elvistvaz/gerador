...
import { NFList, NFId } from '../models/nF.model';
import { NFService } from '../services/nF.service';
import { SessionService } from '../services/session.service';

@Component({
  selector: 'app-nF-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './nF-list.component.html',
  styleUrls: ['./nF-list.component.css']
})
export class NFListComponent implements OnInit {

  items: NFList[] = [];
  filteredItems: NFList[] = [];
  searchTerm: string = '';
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/nF/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/nF/novo']);
    }
  }

  editar(id: NFId): void {
    const idParam = Object.values(id).join(',');
    this.router.navigate(['/nF/editar', idParam]);
  }
