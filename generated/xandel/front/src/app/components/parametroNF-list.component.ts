...
import { ParametroNFList } from '../models/parametroNF.model';
import { ParametroNFService } from '../services/parametroNF.service';

@Component({
  selector: 'app-parametroNF-list',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './parametroNF-list.component.html',
  styleUrls: ['./parametroNF-list.component.css']
})
export class ParametroNFListComponent implements OnInit {

  items: ParametroNFList[] = [];
  filteredItems: ParametroNFList[] = [];
  searchTerm: string = '';
  ...
  constructor(
    private service: ParametroNFService,
  ...
  novo(): void {
    ...
    if (Object.keys(this.parentFilter).length > 0) {
      this.router.navigate(['/parametroNF/novo'], { queryParams: this.parentFilter });
    } else {
      this.router.navigate(['/parametroNF/novo']);
    }
  }

  editar(id: number): void {
    this.router.navigate(['/parametroNF/editar', id]);
  }
