...
// Paths
const jsonPathEntities = path.join(__dirname, 'metamodel', 'data', 'xandel', 'xandel-entities.json');
const jsonPathModel = path.join(__dirname, 'metamodel', 'data', 'xandel', 'xandel-model.json');
const csvDir = path.join(__dirname, 'metamodel', 'data', 'xandel', 'carga-inicial');

// Read JSON files
const jsonEntities = JSON.parse(fs.readFileSync(jsonPathEntities, 'utf8'));
const jsonModel = JSON.parse(fs.readFileSync(jsonPathModel, 'utf8'));

// Merge entities from both files
const entitiesContinuation = jsonEntities.entities_continuation || [];
const entitiesMain = jsonModel.entities || [];
const entities = [...entitiesMain, ...entitiesContinuation];
...
console.log('RELATÓRIO DE COMPATIBILIDADE - JSON vs CSV (COMPLETO)');
console.log('='.repeat(100));
console.log();

// Statistics
console.log(`Total de entidades no JSON (xandel-model.json): ${entitiesMain.length}`);
console.log(`Total de entidades no JSON (xandel-entities.json): ${entitiesContinuation.length}`);
...
entities.forEach(entity => {
  ...
  try {
    ...
    if (missingInCSV.length > 0 || extraInCSV.length > 0) {
      ...
      results.perfectMatch.push(entityName);
      console.log();
    }
...
console.log(`✅ Entidades com match perfeito (todos os campos): ${results.perfectMatch.length}/${entities.length}`);
...
// Additional analysis - list perfect matches
if (results.perfectMatch.length > 0) {
  console.log('✅ ENTIDADES COM MATCH PERFEITO (100% compatibilidade):');
  results.perfectMatch.forEach(entity => {
    console.log(`  - ${entity}`);
  });
  console.log();
}
